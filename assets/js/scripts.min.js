...
init();})(window.publiiThemeMenuConfig);(function(){var menuCssClass='navbar__menu';if(!document.querySelector('.'+menuCssClass)){return;}
var activeMenuItems=document.querySelector('.'+menuCssClass).querySelectorAll('.active');for(var i=0;i<activeMenuItems.length;i++){var activeMenuItem=activeMenuItems[i];var elementToCheck=activeMenuItem.parentNode;while(!elementToCheck.classList.contains(menuCssClass)){if(elementToCheck.classList.contains('has-submenu')){elementToCheck.classList.add('active-parent');}
elementToCheck=elementToCheck.parentNode;}}})();var comments=document.querySelector(".js-post__comments-button");if(comments){comments.addEventListener("click",function(){comments.classList.toggle("is-hidden");var container=document.querySelector(".js-post__comments-inner");container.classList.toggle("is-visible");});}
